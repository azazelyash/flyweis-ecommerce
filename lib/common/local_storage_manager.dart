...
class LocalStorageManager {
  ...
  Future<void> saveAccessToken({required String token}) async {
    try {
      ...
      await secureStorage.write(key: LocalStorageKeys.accessToken.asString, value: token);
    } catch (error) {
  ...
  Future<String?> getAccessToken() async {
    try {
      final token = await secureStorage.read(key: LocalStorageKeys.accessToken.asString);
      if (token != null) {
